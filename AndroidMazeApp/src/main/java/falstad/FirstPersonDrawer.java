...
import williameskay.cs301.cs.wm.edu.amazebywilliameskayandreillymaw.falstad.Wizard.CardinalDirection;
import williameskay.cs301.cs.wm.edu.amazebywilliameskayandreillymaw.generation.BSPBranch;
import williameskay.cs301.cs.wm.edu.amazebywilliameskayandreillymaw.generation.BSPLeaf;
...
import williameskay.cs301.cs.wm.edu.amazebywilliameskayandreillymaw.generation.Seg;
...
public class FirstPersonDrawer extends DefaultViewer implements Serializable {
     ...
     * @param bsp_root
     */
    public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, BSPNode bsp_root, MazeController maze){
        view_width = width ;
        view_height = height ;
    ...
    private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
        ...
        int x1i = x1;
        int xd = x2-x1;
        mazePanel.setSegColor(seg.col, seg.val1);
        boolean drawn = false;
...
//=======================================================Direction Arrow Implementation Methods===========================================//
    ...
    public void drawDirectionArrow(){
        try {
            ...
            CardinalDirection cardDirectionToMove;
            ...
            if (directionToMove == Direction.FORWARD){
                mazePanel.fillRect(195, 70, 10, 30);
                mazePanel.fillPolygon(new int[]{185,215,200}, new int[]{70,70,50}, 3); //up arrow
            }

            if (directionToMove == Direction.BACKWARD){
                mazePanel.fillRect(195, 110, 10, 30);
                mazePanel.fillPolygon(new int[]{185,215,200}, new int[]{140,140,160}, 3); //down arrow
            }

            if (directionToMove == Direction.LEFT){
                mazePanel.fillRect(165, 100, 30, 10);
                mazePanel.fillPolygon(new int[]{165,165,145}, new int[]{90,120,105}, 3); //left arrow
            }

            if (directionToMove == Direction.RIGHT){
                mazePanel.fillRect(205, 100, 30, 10);
                mazePanel.fillPolygon(new int[]{235,235,255}, new int[]{90,120,105}, 3); //right arrow
            }
     ...
     */
    private Direction moveCardDirection(CardinalDirection directionToMove) throws Exception {
        int dx = maze.dx;
        int dy = maze.dy;
